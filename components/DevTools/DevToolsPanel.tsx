...
function seedFakeSessions(count: number) {
  ...
  while (seeded < count) {
    ...
    // Set first session date to earliest
    const existing = localStorage.getItem(FIRST_SESSION_KEY);
    if (!existing || key < existing) {
      localStorage.setItem(FIRST_SESSION_KEY, key);
    }
  }
...
function setWeekNumber(targetWeek: number) {
  const today = new Date();
  const weeksBack = targetWeek - 1;
  const fakeFirst = startOfWeek(subWeeks(today, weeksBack), { weekStartsOn: 1 });
  localStorage.setItem(FIRST_SESSION_KEY, formatDateKey(fakeFirst));
}
